    ...
    <main className="mt-24 min-h-screen pt-24 md:pt-32 pb-16 md:pb-20">
      <Container className="max-w-[1400px]">
        <div className="text-center mb-12 md:mb-16 px-4">
          <p className="uppercase text-md font-bold tracking-widest text-roman-silver mb-3 md:mb-4">
          ...
          <p className="text-roman-silver max-w-4xl mx-auto text-base md:text-xl px-4">
            Browse through our diverse collection of projects spanning cinematography, video editing, animations, and photography.
          </p>
        </div>
        ...
        {/* Portfolio Grid */}
        <div 
          className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 px-4 transition-opacity duration-300 ${
        ...
        {filteredItems.length === 0 && (
          <div className="text-center py-16 md:py-20">
            <p className="text-roman-silver text-lg md:text-xl">No items found in this category.</p>
          </div>
        )}
      </Container>
    </main>
...
function PortfolioCard({ 
  ...
  return (
    <div 
      className={`group relative rounded-xl md:rounded-2xl overflow-hidden bg-raisin-black shadow-xl hover:shadow-2xl transition-all duration-500 hover:scale-[1.02] md:hover:scale-105 ${
        item.isVertical ? 'aspect-[9/16]' : 'aspect-video'
      }`}
      style={{
        animation: !isAnimating ? `fadeInUp 0.6s ease-out ${index * 0.05}s both` : 'none'
      }}
    >
      {item.type === 'video' ? (
        <div className="relative w-full h-full">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
            <div className="absolute bottom-0 left-0 right-0 p-4 md:p-6">
              <h3 className="text-white text-base md:text-xl font-bold">{item.title}</h3>
              <p className="text-gray-300 text-xs md:text-sm mt-1 capitalize">{item.category}</p>
            </div>
          </div>
        </div>
      ) : (
