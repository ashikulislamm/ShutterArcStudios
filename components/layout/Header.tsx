    ...
    <header className="fixed top-0 left-0 w-full z-40 py-6 ">
      <div className="container max-w-[1300px] mx-auto px-4 ">
        ...
        <nav className="hidden lg:flex items-center justify-between bg-raisin-black/80 backdrop-blur-lg rounded-full px-6 py-4 shadow-2xl border border-white-10 ">
          {/* Logo */}
          <Link href="/" className="flex items-center justify-center w-18 h-18 bg-black rounded-full hover:scale-110 transition-transform">
            <Image 
              src="/images/Logo.png" 
        ...
        <div className="lg:hidden flex items-center justify-between bg-raisin-black/80 backdrop-blur-lg rounded-full px-4 py-3 shadow-2xl border border-white-10">
          <Link href="/" className="flex items-center justify-center w-10 h-10 bg-black rounded-full">
            <Image 
              src="/images/Logo.png" 
              alt="ShutterArc Studios" 
              width={28} 
          ...
          <button 
            onClick={toggleNav}
            className="w-10 h-10 bg-crimson-red rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110"
        ...
        {/* Mobile Menu Overlay */}
        <nav 
          className={`lg:hidden fixed top-28 left-4 right-4 bg-raisin-black/90 backdrop-blur-xl rounded-3xl p-8 transition-all duration-300 border border-white-10 shadow-2xl ${
            isNavOpen ? 'translate-y-0 opacity-100 visible' : '-translate-y-4 opacity-0 invisible'
          }`}
        >
            ...
            {NAV_LINKS.map((link) => (
              <li key={link.href}>
                <Link 
                  href={link.href}
                  onClick={closeNav}
                  className="block text-white text-base font-medium py-3 px-4 rounded-xl hover:bg-white-5 hover:text-crimson-red transition-all"
                >
                  {link.label}
                </Link>
              </li>
